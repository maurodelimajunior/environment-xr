// Mesma estrutura que main_local.js, mas com integração de tempo real
...
const MODELS = [
  { id: "escultura", url: "https://seuservidor.com/escultura.glb", animated:true },
  { id: "quadro", url: "https://seuservidor.com/quadro.glb", animated:false }
];
...
async function resolveCloudAnchor() {
  ...
  const resolvedPose = await resolveCloudAnchorNative(data.id);
...
window.addEventListener("click", async () => {
  if(!isAdmin) return;
  const newAnchorId = await createCloudAnchorNative(session);
  const modelsData = {};
  Object.keys(modelsLoaded).forEach(id => {
    const model = modelsLoaded[id];
    modelsData[id] = {
